...
#### 设计原则：1）FindClusters ≠ 可视化，2）主 / 亚聚类结果必须结构化返回，同时兼顾后续主图聚类和细胞亚群分析，3）代码级safety check 强制中止，提前暴露逻辑错误。

自定义 P5_clustering_umap_module 函数模块，已修正在 Seurat v4 / v5 都安全的标准函数。
主流程中调用方法如下：
```
